...
Если окно плагина в Figma закрыто, агент попытается открыть его автоматически при первом `plugin`-событии с контроллера (энкодер/кнопка, замапленные на plugin-команду).
Для кнопок типа `hotkey` авто-открытие плагина не используется.
Для авто-открытия у Terminal/iTerm должны быть macOS-разрешения: **Privacy & Security → Accessibility** и **Automation (Terminal/iTerm → Figma)**.
